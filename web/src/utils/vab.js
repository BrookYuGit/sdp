...
const install = (Vue, opts = {}) => {
  ...
  Vue.prototype.$baseDiffTsp = (tsp1, tsp2) => {
    let mDiff = dayjs(tsp1).diff(tsp2, 'm', true).toFixed(2)
    if (mDiff > 60) {
      mDiff = dayjs(tsp1).diff(tsp2, 'h', true).toFixed(2) + '小时'
    } else if (mDiff < 1) {
      mDiff = dayjs(tsp1).diff(tsp2, 's', true).toFixed(2) + '秒'
    } else {
      mDiff += '分钟'
    }
    return mDiff
