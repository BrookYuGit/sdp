...
;(function (mod) {
  ...
  CodeMirror.defineMode('clike', function (config, parserConfig) {
    var indentUnit = config.indentUnit,
      ...
      sdpNormalKeywords = parserConfig.sdpNormalKeywords || {},
      sdpPropKeywords = parserConfig.sdpPropKeywords || {},
      sdpBlockKeywords = parserConfig.sdpBlockKeywords || {},
    ...
    function tokenBase(stream, state) {
      ...
      if (contains(sdpBlockKeywords, cur)) {
      ...
      if (contains(sdpPropKeywords, cur)) {
        stream.backUp(cur.length)
        if (!stream.sol()) {
          stream.backUp(1)
          let ch2 = stream.peek()
      ...
      if (contains(sdpNormalKeywords, cur)) {
        stream.backUp(cur.length)
        if (!stream.sol()) {
          stream.backUp(1)
          let ch2 = stream.peek()
  ...
  def('text/x-java', {
    ...
    sdpNormalKeywords: words(window.cmSdp.keywords),
    sdpBlockKeywords: words(window.cmSdp.blockKeywords),
    sdpPropKeywords: words(window.cmSdp.propKeywords),
    types: words(
