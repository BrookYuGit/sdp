...
public class DatabaseIntrospector {
    ...
    private void calculateUniqueKey(FullyQualifiedTable table,
                                    IntrospectedTable introspectedTable) throws Exception {
        ResultSet rs = null;
        ...
        try {
            rs = databaseMetaData.getIndexInfo(
                    table.getIntrospectedCatalog(), table
                            ...
                            .getIntrospectedTableName(), true, false);
        ...
        try {
            ...
            Map<String, Map<Short, String>> keyColumns = new HashMap<>();
            Map<Short, String> primaryKeyColumns = new TreeMap<>();
            ResultSetMetaData metaData = rs.getMetaData();
            while (rs.next()) {
                String indexName = rs.getString("INDEX_NAME").toLowerCase(); //$NON-NLS-1$
                String columnName = rs.getString("COLUMN_NAME").toLowerCase(); //$NON-NLS-1$
                short keySeq = rs.getShort("ORDINAL_POSITION"); //$NON-NLS-1$
                if ("primary".equals(indexName) || indexName.startsWith("primary_key_")) {
                    primaryKeyColumns.put(keySeq, columnName);
                    continue;
                }
                if (!keyColumns.containsKey(indexName)) {
                    keyColumns.put(indexName, new TreeMap<>());
                }
                keyColumns.get(indexName).put(keySeq, columnName);
            }

            for (String indexName: keyColumns.keySet()) {
                for (String columnName : keyColumns.get(indexName).values()) {
                    introspectedTable.addUniqueColumn(indexName, columnName);
                }
            }

            for (String columnName : primaryKeyColumns.values()) {
    ...
    private void reportIntrospectionWarnings(
        ...
        for (ColumnOverride columnOverride : tableConfiguration
                .getColumnOverrides()) {
            if (introspectedTable.getColumn(columnOverride.getColumnName().toLowerCase()) == null) {
                warnings.add(getString("Warning.3", //$NON-NLS-1$
                        columnOverride.getColumnName().toLowerCase(), table.toString()));
            }
        }
    ...
    public List<IntrospectedTable> introspectTables(TableConfiguration tc)
            throws Exception {
        return introspectTables(tc, 1);
    }

    public List<IntrospectedTable> introspectTables(TableConfiguration tc, int fixParameter)
            throws Exception {

//       System.out.println("**** introspectTables:"+tc.getCatalog()+"."+tc.getTableName()+","+fixParameter);
            ...
            for (IntrospectedColumn introspectedColumn : entry.getValue()) {
                ...
                if (isTrue(tc
                    ...
                    sb.append(getCamelCaseString(
                            introspectedColumn.getFirstRemarks(), true));
                    introspectedColumn.setJavaProperty(
    ...
    private Map<ActualTableName, List<IntrospectedColumn>> getColumns(
        ...
        boolean hasFirstAndIsFirst = rs.next();
        if (!hasFirstAndIsFirst) {
            closeResultSet(rs);
            rs = databaseMetaData.getColumns(null, localSchema,
                    localTableName, "%"); //$NON-NLS-1$
            hasFirstAndIsFirst = rs.next();
        }
        ...
        for (int i = 1; i <= colCount; i++) {
            if ("IS_AUTOINCREMENT".toLowerCase().equals(rsmd.getColumnName(i).toLowerCase())) { //$NON-NLS-1$
                supportsIsAutoIncrement = true;
            }
            if ("IS_GENERATEDCOLUMN".toLowerCase().equals(rsmd.getColumnName(i).toLowerCase())) { //$NON-NLS-1$
                supportsIsGeneratedColumn = true;
            }
        }

        while (hasFirstAndIsFirst || rs.next()) {
            hasFirstAndIsFirst = false;
            ...
            if ("JSON".equals(typeName) || "TEXT".equals(typeName)) {
                introspectedColumn.setJdbcType(Types.LONGVARCHAR);
            }
    ...
    private List<IntrospectedTable> calculateIntrospectedTables(
            ...
            Map<ActualTableName, List<IntrospectedColumn>> columns) throws Exception {
        boolean delimitIdentifiers = tc.isDelimitIdentifiers()
                || stringContainsSpace(tc.getCatalog())
                || stringContainsSpace(tc.getSchema())
                || stringContainsSpace(tc.getTableName());
...
//            calculatePrimaryKey(table, introspectedTable);
    ...
    private void enhanceIntrospectedTable(IntrospectedTable introspectedTable) {
        try {
            ...
            if (rs.next()) {
                throw new SQLException("multi table found:"+fqt.getIntrospectedTableName());
            }
            closeResultSet(rs);
        } catch (SQLException e) {
